...
public class RESTTransferServiceTest {
    ...
    public void transferTest() throws IOException {
        ...
        response = restTransferService.transfer(null, mike.getId() + boris.getId(), boris.getId(), BigDecimal.valueOf(10));
        assertEquals(response.getStatus(), FORBIDDEN.getStatusCode());
        ...
        response = restTransferService.transfer(null, mike.getId(), mike.getId() + boris.getId() + 1, BigDecimal.valueOf(10));
        assertEquals(response.getStatus(), FORBIDDEN.getStatusCode());

        //test transfer to the same account
        response = restTransferService.transfer(null, mike.getId(), mike.getId(), BigDecimal.valueOf(10));
        assertEquals(response.getStatus(), FORBIDDEN.getStatusCode());
        ...
        response = restTransferService.transfer(null, mike.getId(), boris.getId(), BigDecimal.valueOf(-1));
        assertEquals(response.getStatus(), FORBIDDEN.getStatusCode());

        //test not enough money
        response = restTransferService.transfer(null, mike.getId(), boris.getId(), BigDecimal.valueOf(200));
        assertEquals(response.getStatus(), FORBIDDEN.getStatusCode());

        //test transfer succeeded and check for money lose
        response = restTransferService.transfer(null, mike.getId(), boris.getId(), BigDecimal.valueOf(50));
        assertEquals(response.getStatus(), OK.getStatusCode());
        assertEquals(mike.getBalance().add(boris.getBalance()),
                accountService.findById(mike.getId()).getBalance().add(accountService.findById(boris.getId()).getBalance()));
        assertEquals(mike.getBalance().subtract(BigDecimal.valueOf(50)), accountService.findById(mike.getId()).getBalance());
